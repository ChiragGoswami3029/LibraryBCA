...
def upload_file():
    ...
    record = FileItem(
        ...
        pdf_url=upload_result["secure_url"],   # type: ignore
        cloudinary_public_id=upload_result["public_id"], # type: ignore
...
def download_file(file_id):
    record = FileItem.query.get_or_404(file_id)
    return redirect(record.pdf_url)
...
def delete_file(file_id):
    ...
    try:
        cloudinary.uploader.destroy(record.cloudinary_public_id)
    except Exception:
        pass # Ensure DB record is deleted even if Cloudinary fails
